...
$(function () {
    ...
    loadBuyShowAndComment(0, 0);

    //
    $(".comment-header li .comment_type").unbind('click').click(function () {
        loadBuyShowAndComment($(".comment-header option:first").val(), $(this).prev().val())
    })
    $(".comment-header li select").unbind('click').click(function () {
        $("#evaluate_modal").modal("show");
    ...
    $('.uploader-holder .upload-img-box .file-input').localResizeIMG({
        ...
        success: function (result) {
            var str = '<li class="upload-img-box">' +
                ...
                '</div>' +
                //'<input type="file" class="img" name="file" value="'+img.src+'"/>'+
                '</li>';
    ...
    function loadSubmitEvaluateBtn() {
        $("#submit_evaluate").off("click").on("click", function () {
            var uploadForm = new FormData();
            for (var i = 0; i < picArr.length; i++) {
                uploadForm.append("file", picArr[i]);
            }
            $.ajax({
    ...
    //加载买家秀以及评论
    function loadBuyShowAndComment(order, commentType) {
        $.ajax({
            url: urlPrefix + "/list_buyShow",
            method: "get",
            data: "pageNum=0&pageSize=" + buyShowSize + "&replySize=" + defaultreplySize + "&goodsId=" + goodsId + "&dateTime=" +
                refreshTime + "&order=" + order + "&commentType=" + commentType,
            success: function (result) {
                if (result.code == 200) {
                    if (result.data.list.length > 0) {
                        ...
                        if (result.data.total > buyShowSize) {//超过默认条数才显示分页
                            str = '<div id="buyShow_page" class="normal" align="left" style="padding:10px 0px"></div>';
                            $("#Comment").append(str);
        ...
        $.each(data.list, function (index, obj) {
            if (obj.grade == "1") {//根据评论等级初始化
                ...
                var str = '<div data-id=' + obj.comId + ' class="reply-wrap">' + //父级评论编号
                    ...
                    '                            <ul>\n' +
                    '                              <li class="">\n' +
                    '                                <img src="assets/pages/img/products/model5.jpg"/>\n' +
                    '                              </li>\n' +
                    '                              <li class="">\n' +
        ...
        $("#buyShow_page").pagination(pageInfo.total, { //第一个参数指定共多少条记录
            ...
            callback: function (index) {
                ...
                $.ajax({
                    url: urlPrefix + "/list_buyShow",
                    method: "get",
                    data: "pageNum=" + pageNum + "&pageSize=" + buyShowSize + "&replySize=" + defaultreplySize + "&goodsId=" + goodsId + "&dateTime=" +
                        refreshTime + "&order=0",
                    success: function (result) {
                        if (result.code == 200) {
                            if (result.data.list.length > 0) {
    ...
    function loadReplyReportClick() {
        ...
        //鼠标滑入回复评论显示举报按钮
        $(".reply-box .reply-item").off("mouseenter").on("mouseenter", function () {
            $(this).find(".report").css({"display": "block"});
        });
        //鼠标滑出回复评论隐藏举报按钮
        $(".reply-box .reply-item").off("mouseleave").on("mouseleave", function () {
    ...
    function loadCommentSubmitBtn($rplyBtn, byReplyName) {
        ...
        $(".comment-send .comment-submit").off("click").on("click", function () {
            ...
            var str = '<div class="reply-item" data-cid="2003">' + //回复评论编号
                ...
                '                          </div>';
            var commentData = {
                "goodsId": goodsId, "parentId": $btn.attr("data-parentCid"),
